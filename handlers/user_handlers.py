...
async def start_cmd(message: types.Message, state: FSMContext, session: AsyncSession, i18n: I18nContext) -> None:
    ...
    if user_in_db:
        ...
        await message.answer(i18n.get("I am your days abroad counter bot"), reply_markup=MENU_KEYBOARD)
    else:
        await message.bot.delete_my_commands(scope=types.BotCommandScopeChat(chat_id=message.chat.id))
        await message.bot.set_my_commands(commands=INITIAL_MENU_ITEMS, scope=types.BotCommandScopeChat(chat_id=message.chat.id))
...
async def switch_language(message: types.Message, i18n: I18nContext, session: AsyncSession, locale_code: str, update_db: bool) -> None:
    ...
    if update_db:
        try:
            ...
            await message.answer(i18n.get("Sorry, I'm on maintenance. Please try again later"))
    
        if locale_code == "en":
            await message.answer("Language switched to: " + locale_code, reply_markup=MENU_KEYBOARD)
        else:
            await message.answer("Язык переключен на : " + locale_code, reply_markup=MENU_KEYBOARD)


@user_handlers_router.message(Command("en"))
async def switch_to_en(message: types.Message, session: AsyncSession, i18n: I18nContext) -> None:
    await switch_language(message, i18n, session, "en", True)


@user_handlers_router.message(Command("ru"))
async def switch_to_en(message: types.Message, session: AsyncSession, i18n: I18nContext) -> None:
    await switch_language(message, i18n, session, "ru", True)
...
async def add_trip_callback(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, i18n: I18nContext) -> None:
    ...
    await callback.message.answer(i18n.get("Add start date"), reply_markup=await MySimpleCalendar().start_calendar())
...
async def change_trip_callback(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, i18n: I18nContext) -> None:
    ...
    if trip_buttons != 0:
        ...
        await callback.message.answer(i18n.get("Select a trip"), reply_markup=trips_to_change)
    else:
        await callback.message.answer(i18n.get("There are no trips to edit"), reply_markup=MENU_KEYBOARD)
...
async def delete_trip_callback(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, i18n: I18nContext) -> None:
    ...
    if trip_buttons != 0:
        ...
        await callback.message.answer(i18n.get("There are no trips to remove"), reply_markup=MENU_KEYBOARD)
        return
...
@user_handlers_router.callback_query(AddChangeTrip.select_trip, or_f(F.data.contains("prev_page"), F.data.contains("next_page")))
...
async def export_trips_callback(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, i18n: I18nContext) -> None:
    ...
    try:
        ...
        await callback.message.answer(i18n.get("Something went wrong. Please start over"), reply_markup=MENU_KEYBOARD)
        return
    ...
    if os.path.exists(exported_trips):
        ...
        await callback.message.answer(i18n.get("Something went wrong. Please try again"), reply_markup=MENU_KEYBOARD)
...
async def add_user_config_one_year_limit(message: types.Message, state: FSMContext, session: AsyncSession, i18n: I18nContext) -> None:
    if message.text == "." and AddChangeUserConfig.user_to_change:
        ...
        await message.answer(i18n.get("Please enter valid number of days. If you want to only use a limit for two years, send 0"))
        return
...
async def add_user_config_two_years_limit(message: types.Message, state: FSMContext, session: AsyncSession, i18n: I18nContext) -> None:
    if message.text == "." and AddChangeUserConfig.user_to_change:
        ...
        await message.answer(i18n.get("Please enter valid number of days"))
        return
...
async def select_language_en_callback(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, i18n: I18nContext) -> None:
    ...
    if AddChangeUserConfig.user_to_change:
        ...
        await switch_language(callback.message, i18n, session, language, False)
        await callback.message.answer(i18n.get("Your settings have been updated"), reply_markup=MENU_KEYBOARD)
    else:
        obj = {
        ...
        await switch_language(callback.message, i18n, language, False)
...
async def change_product_callback(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, i18n: I18nContext) -> None:
    ...
    await callback.message.answer(i18n.get("Add new start date"), reply_markup=await MySimpleCalendar().start_calendar(int(startdate.year), int(startdate.month)))
    await state.set_state(AddChangeTrip.add_start_date)
...
async def delete_product_callback(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, i18n: I18nContext) -> None:
    ...
    await callback.message.answer(i18n.get("The trip has been removed"), reply_markup=MENU_KEYBOARD)
...
async def trip_start_date_callback(callback: types.CallbackQuery, callback_data: CallbackData, state: FSMContext, session: AsyncSession, i18n: I18nContext) -> None:
    calendar = MySimpleCalendar(show_alerts=False)
    ...
    if selected:
        ...
        await callback.message.answer(message_text, reply_markup=await MySimpleCalendar().start_calendar(date_year, date_month))
...
async def trip_start_date(message: types.Message, state: FSMContext, session: AsyncSession, i18n: I18nContext) -> None:
    ...
    if message.text == "." and AddChangeTrip.trip_to_change:
        ...
        if not is_valid_date_format(message.text):
            await message.answer(i18n.get("The information you entered is not a valid date. Please use the following format: dd-mm-yy"))
            return
        input_datetime = datetime.strptime(message.text, '%d-%m-%y')
        await state.update_data(startdate=input_datetime)
    
    await message.answer(i18n.get("Add end date"))
...
async def trip_end_date_callback(callback: types.CallbackQuery, callback_data: CallbackData, state: FSMContext, session: AsyncSession, i18n: I18nContext) -> None:    
    ...
    if selected:
        if not is_date_after(data['startdate'], date):
            await callback.message.answer(i18n.get("The date you entered is before the start date of your trip. Please enter a different date"))
            return
        else:
            await state.update_data(enddate=date)
            await callback.message.answer(i18n.get("Add description"))
...
async def trip_end_date(message: types.Message, state: FSMContext, session: AsyncSession, i18n: I18nContext) -> None:
    ...
    if message.text == "." and AddChangeTrip.trip_to_change:
        ...
        if not is_date_after(data['startdate'], datetime.strptime(message.text, '%d-%m-%y')):
            await message.answer(i18n.get("The date you entered is before the start date of your trip. Please enter a different date"))
            return
        input_datetime = datetime.strptime(message.text, '%d-%m-%y')
        await state.update_data(enddate=input_datetime)
    
    await message.answer(i18n.get("Add description"))
...
async def trip_description(message: types.Message, state: FSMContext, session: AsyncSession, i18n: I18nContext) -> None:
    ...
    try:
        if AddChangeTrip.trip_to_change:
            ...
            AddChangeTrip.trip_to_change = None
            #не работает перевод
            message_text = i18n.get("Trip changed. Number of your days abroad: {days_abroad}")
        else:
            await orm_add_trip(session, data)
            #не работает перевод
            message_text = i18n.get("Trip added. Number of your days abroad: {days_abroad}")
            
        days_abroad = await calculate_number_of_days(session=session, userid=message.from_user.id)
        if days_abroad != -1:
            await message.answer(message_text.format(days_abroad=days_abroad))
            await message.answer(i18n.get("Here is the menu"), reply_markup=MENU_KEYBOARD)
        else:
            await message.answer(i18n.get("Sorry, I'm on maintenance. Please try again later"))
        
    except Exception as e:
        await message.answer(i18n.get("Something went wrong. Please start over"), reply_markup=MENU_KEYBOARD)
